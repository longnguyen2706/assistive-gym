...
class HumanUrdf(Agent):
    def __init__(self, controllable_joints='right'):
        ...
        self.action_duplication = [1, 1, 1, 4, 1] if 'wheel' in controllable_joints else [1, 4,
                                                                                          1]  # The urdf models the prismatic arm as multiple joints, but we want only 1 action to control all of them.
        ...
        super(HumanUrdf, self).__init__(controllable_joints, right_arm_joint_indices, left_arm_joint_indices,
                                      wheel_joint_indices, right_end_effector, left_end_effector, right_gripper_indices,
                                      left_gripper_indices, gripper_pos, right_tool_joint, left_tool_joint, tool_pos_offset,
                                      tool_orient_offset, right_gripper_collision_indices, left_gripper_collision_indices,
                                      toc_base_pos_offset, toc_ee_orient_rpy, wheelchair_mounted, half_range=False,
                                      action_duplication=self.action_duplication, action_multiplier=self.action_multiplier,
                                      flags='stretch')
    ...
    def init(self, directory, id, np_random, fixed_base=False):
        ...
        self.body = p.loadURDF(os.path.join(directory, 'human', 'human_pip.urdf'), useFixedBase=False,
                               basePosition=[-1, -1, 0.09], physicsClientId=id)

        print ('Human body:', self.body)
        super(HumanUrdf, self).init(self.body, id, np_random)
