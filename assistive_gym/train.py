...
def cal_energy_change(env, original_link_positions, current_link_positions):
    ...
    print(f"Total energy change: {total_energy_change}", f"Potential energy initial: {potential_energy_initial}", f"Potential energy final: {potential_energy_final}")

    return total_energy_change, potential_energy_initial, potential_energy_final

def cost_fn(env, solution, target_pos, end_effector="right_hand", is_self_collision = False, is_env_collision= False,  energy_final = 0):
    human = env.human
...
def generate_target_points(env, num_points):
    ...
    points = fibonacci_evenly_sampling_range_sphere(right_hand_pos, [0.25, 0.5], 100)
...
def cal_cost(env, solution, target,  original_self_collisions, original_collisions, env_object_ids, original_link_positions):
    ...
    energy_change, energy_original, energy_final = cal_energy_change(env, original_link_positions, cur_link_positions)
    cost, m, dist = cost_fn(env, solution, target, is_self_collision=is_self_collision, is_env_collision=is_env_collision,
                            energy_final=energy_final)
    return cost, m, dist, energy_final
...
def train(env_name, seed=0,  num_points = 50, smpl_file = 'examples/data/smpl_bp_ros_smpl_re2.pkl', save_dir='./trained_models/', render=False):
    ...
    for (idx, target) in enumerate(points):
        ...
        optimizer = init_optimizer(x0, sigma=0.5)
        timestep = 0
