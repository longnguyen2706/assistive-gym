...
from assistive_gym.envs.utils.urdf_utils import generate_human_mesh, load_smpl
...
def generate_urdf(args):
    ...
    smpl_data = load_smpl(args.smpl_file)
    print("###\nBetas before: ", smpl_data.betas)
    smpl_data.betas = [(np.random.rand((len(smpl_data.betas[0]))) - 0.5) * 2]
    # smpl_data.betas[0][2] += 10
    print("betas after: ", smpl_data.betas[0], "\n")

    
    # generate_human_mesh(physic_client_id, gender, urdf_ref_file, urdf_folder, args.smpl_file)
    generate_human_mesh(physic_client_id, gender, urdf_ref_file, urdf_folder, smpl_data=smpl_data)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--person-id', default='p000', help='Person id')
    parser.add_argument('--smpl-file', default='examples/data/slp3d/p000/s01.pkl',
